...
- Two or more VCF files generated by [sawfish](https://github.com/PacificBiosciences/sawfish) (multi-sample inputs are supported; each input must contain at least one sample)
...
## One-shot vs incremental merges

For best consistency, prefer a one-shot merge of all inputs in a single run. Incremental (multi-stage) merging can introduce small precision drift on CI-sensitive fields after repeated write/read cycles, especially `CIPOS`/`CIEND` and occasionally downstream `POS`/`END`/`SVLEN` by `+/-1` on some records. If exact parity is important do a single shot merge.
...
For multi-sample inputs svx treats each input VCF record as one atomic unit and does not split records into per-sample records before clustering. This supports both joint-called multi-sample sawfish outputs and incremental merge workflows.
...
- `--sort-output` enables global output sorting (`RID`, `POS`, case-sensitive allele lexical tie key)
  - When `--sort-output` is used with a compressed file output (for example, `-o out.vcf.gz`), svx automatically builds a matching index (`.tbi` for VCF, `.csi` for BCF) after final sorted emission.
- `--sort-max-mem` sets the sorter memory cap (default `768M`; suffixes `k/m/g` use decimal units)
- `--sort-tmp-dir` sets the temporary spill root for sorted output; the directory must already exist
- `--min-supp` sets the minimum supporting-sample count (`SUPP`) required to write a merged record (default `1`)
- `--keep-monomorphic` allows writing monomorphic merged records with `SUPP=0` (by default these are not written)
- `--print-header` prints the merged header and exits
- `--no-version` omits `##svxVersion` and `##svxCommand` header lines

### How `--sort-output` works

When `--sort-output` is enabled, svx performs global sorting in the writer stage:

- Each merged blob is materialized as records and ordered by the output comparator (`RID`, `POS`, then case-sensitive allele lexical tie key).
- Records are buffered in memory and pre-spill interleaving is used while under `--sort-max-mem`.
- If buffered data exceeds `--sort-max-mem`, sorted runs are spilled to disk under `--sort-tmp-dir` (or the system temp directory if not set).
- At the end of merge runtime, svx performs final sorted emission:
  - If no spill runs exist, buffered records are drained directly.
  - If spill runs exist, remaining buffered records are spilled and a streaming k-way merge writes final output.
- The spill-sort implementation is maintained in `crates/hts-spill-sort`; user-facing sort flags and semantics remain the same.

Any sort validation/spill/merge/cleanup failure is treated as an error; svx does not fall back to unsorted output.
...
The default is `--svtype ALL`, which includes `INS`,`DEL`,`INV`,`DUP`,`BND`,`CNV`. For `--svtype CNV`, svx treats CNV records as copy-number loci across samples and writes canonical CNV output records (`SVTYPE=CNV`, `ALT=<CNV>`, non-directional `SVLEN`) so multi-stage merges remain representation-stable. See [Output interpretation](output.md) for details.
